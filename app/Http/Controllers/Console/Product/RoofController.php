...
class RoofController extends Controller
{
    ...
    protected function store(Request $request)
    {
        ...
        foreach ($validated['product_attributes'] as $key => $attribute) {
            if (($attribute['price'] + $roofPrice) < 0) {
                array_push($attributeError, [$key => 'Inavalid value']);
            } else {
                array_push($attributes, ['thickness' => $attribute['thickness'], 'price' => $attribute['price']]);
            }
        }
    ...
    public function update(Request $request, Product $product)
    {
        ...
        foreach ($validated['product_attributes'] as $key => $attribute) {
            if (($attribute['price'] + $roofPrice) < 0) {
                ...
                if ($attribute['thickness']) array_push($attributes, ['thickness' => $attribute['thickness'], 'price' => $attribute['price']]);
            }
        }
        if (count($attributeError)) return "error";
        unset($validated['product_attributes']);
    ...
    {
        $p = CategoryPrice::where('category_id', 3)->first();
