<form action="{{ route('console.products.other.store') }}" method="post" id="product-form">
    @csrf
    <div class="card mt-5 ">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="mb-10">
                            ...
                            <select class="form-select" aria-label="Select Subcategory" name="category_id"
                                id="category_id">
                    ...
                    <div class="col-md-6">
                        <div class="mb-10">
                            <label for="exampleFormControlInput1" class="required form-label">Subcategory</label>
                            <select class="form-select" aria-label="Select Subcategory" name="subcategory_id"
                                id="subcategory_id">

                                <option value="aaa">aaaaaaaa</option>
                        ...
                        <div data-repeater-item>
                            <div class="form-group row">
                                <div class="col-md-3">
                                    ...
                                    <input type="text" name="thickness" class="form-control mb-2 mb-md-0"
                                        placeholder="Enter thickness" />
                                </div>
                                <div class="col-md-3">
                                    <label class="form-label">weight</label>
                                    <input type="text" name="weight" class="form-control mb-2 mb-md-0"
                                        placeholder="Enter weight" />
                                </div>
                                <div class="col-md-2">
                                    <label class="form-label">Price(+/-)</label>
                                    <input type="text" name="price" class="form-control mb-2 mb-md-0"
                                        placeholder="Enter price" onkeyup="price(this)" />
                                </div>

                                <div class="col-md-2">
                                    <label class="form-label">Pric/Kg</label>
                                    <input type="text" name="kg"
                                        class="form-control mb-2 mb-md-0 category_price" placeholder="Enter Pric/Kg"
                                        disabled />
                                </div>
...
<script>
    ...
    const apple = @json($categoryPrice);
    let categoryPrice = 0;
    document.getElementById('category_id').addEventListener('change', function(e) {
        const s = subcategories.filter((s) => s.category_id == e.target.value);
        const subcategory_id = document.getElementById('subcategory_id');
        subcategory_id.innerHTML = '<option value="" selected >--Select Subcategory--</option>';
        s.forEach(function(item) {
            let option = document.createElement('option'); // Create a new option element
            option.value = item.id; // Set the value attribute
            option.textContent = item.subcategory_name; // Set the visible text

            subcategory_id.appendChild(option); // Append the option to the select box
        });
        subcategory_id.disabled = false;

        console.log(apple)

        const price = apple.find((p) => p.category_id == e.target.value)

        console.log(price)
        const aaa = document.querySelectorAll('.category_price');

        aaa.forEach(element => {
            element.value = price.price
        });

        categoryPrice = price.price;
    })
    ...
    function price(e) {
        ...
        if (isFinite(e.value)) newPrice = Number(categoryPrice) + Number(e.value);
        else newPrice = categoryPrice;
        parent.childNodes[7].childNodes[3].value = newPrice
    }
