    ...
    <table style="width: 100%;">
        ...
        <tr>
            <td style="width: 75%;">
                ...
                font-family: system-ui;margin-bottom: 10px">
                    {{ $settings->site_name }}</h1>
                <span style="font-size: 16px;font-family: sans-serif;">
                    GSTIN : <strong>{{ $settings->gstin }}</strong> <br>
                    {{ $settings->address }}<br>
                    {{ $settings->city }}, {{ $settings->state }} - {{ $settings->pincode }} <br>
                    Phone: +91 {{ $settings->phone }}</span>

            </td>
            <td style="width: 25%;">
                <img src="https://theprintplanet.co.in/steelghar/console/public/storage/images/logo.jpeg" alt=""
    ...
    border-collapse: collapse; margin-top:10px;margin-bottom:10px">
        ...
        @foreach ($orderItems as $item)
            <tr style="border-bottom: 2px solid rgba(131, 129, 129, 0.438);">
                ...
                <td style="width: 15%;text-align: left;">{{ number_format($item->weight, 2) }}</td>
                <td style="width: 5%;text-align: left;">{{ number_format($item->length, 2) }}</td>
